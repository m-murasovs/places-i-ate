...
- [x] Per-visit visibility setting: public / followers-only / private (`visibility String @default("public")` on `Visit`). Enforced on the profile visit list/map (`getPublicProfile` scopes by viewer: owner sees all, follower sees public+followers, others see public) and the place-page "Friends rated" feed (`getPlaceDetail` filters friend visits to public+followers). Aggregate average and leaderboard intentionally stay global. Visibility selector in `VisitForm` + `VisitCard` edit mode (shared `src/lib/visibility.ts`); badge on the card when not public
...
10. **Visibility as a `String` with default, not a Prisma enum** — matches the existing `rating` String convention and avoids a MongoDB enum migration. Enforcement is a query-level `visibility: { in: [...] }` filter computed from the viewer's follow relationship, applied only to the identifiable-content read paths (profile visits, place "Friends rated"). Aggregate average and leaderboard remain global anonymous stats by design — they never reveal who rated, so they don't leak private content
